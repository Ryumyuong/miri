...
import { BLOCK_META, BLOCK_FIELDS, BLOCK_STYLE, isRichField } from "@/lib/itinerary-blocks";
import { usePorts } from "@/lib/usePorts";

const WEEKDAYS = ["일", "월", "화", "수", "목", "금", "토"];
// 텍스트 블록 글자 크기 옵션 → CSS
const SIZE_MAP: Record<string, string> = { "작게": "0.85em", "보통": "1em", "크게": "1.3em", "아주 크게": "1.6em" };
  ...
  return (
    <div>
      <div className="mb-[1em] flex justify-end">
        <button
          onClick={toggleAll}
          className="rounded-full border border-[#0B2A4A1a] px-[1.2em] py-[0.5em] text-[min(1.1088vw,21.289px)] max-[991px]:text-[min(2.8794vw,17.2764px)] max-[501px]:text-[2.796vw] font-semibold text-[#1E4D8B] transition hover:bg-[#1E4D8B]/5"
        >
            ...
            <div key={d.id} className="overflow-hidden rounded-[0.8em] border border-[#0B2A4A1a]">
              ...
              >
                <span className="flex flex-wrap items-center gap-[0.8em]">
                  <span className="rounded-full bg-navy px-[0.9em] py-[0.35em] text-[min(1.0337vw,19.844px)] max-[991px]:text-[min(3.085vw,18.51px)] max-[501px]:text-[2.9957vw] font-bold text-white">
                    {d.dayNo + 1}일차
                  </span>
                  {dayDateLabel(departDate, d.dayNo) && (
                    <span className="text-[min(1.0611vw,20.3735px)] max-[991px]:text-[min(3.1673vw,19.0038px)] max-[501px]:text-[3.0758vw] font-semibold text-slate-500">{dayDateLabel(departDate, d.dayNo)}</span>
                  )}
                  {d.label && <span className="text-[min(1.782vw,34.2144px)] max-[991px]:text-[min(4.6276vw,27.7656px)] max-[501px]:text-[4.4937vw] font-bold text-navy">{d.label}</span>}
                </span>
                <span className={`text-[min(1.0876vw,20.8829px)] max-[991px]:text-[min(3.2465vw,19.479px)] max-[501px]:text-[3.1527vw] text-slate-400 transition ${isOpen ? "rotate-180" : ""}`}>▼</span>
              </button>

              {isOpen && (
                <div className="px-[1.4em] py-[1.4em]">
                  {d.blocks.length === 0 ? (
                    <p className="text-[min(1.386vw,26.6112px)] max-[991px]:text-[min(3.5993vw,21.5958px)] max-[501px]:text-[3.495vw] text-slate-400">등록된 내용이 없습니다.</p>
                  ) : (
...
function BlockRow({ block: b }: { block: ItineraryBlock }) {
  ...
  // 텍스트 블록(제목/내용) 글자 크기·강조 색상
  const textStyle: CSSProperties = {};
  if (b.fontSize && SIZE_MAP[b.fontSize]) textStyle.fontSize = SIZE_MAP[b.fontSize];
              ...
              {fields
                ...
                .map((f) => {
                  ...
                  if (f.isImage) {
                    ...
                    return imgs.length ? (
                      <div key={String(f.key)} className="col-span-2 flex flex-wrap gap-[0.6em]">
                        {imgs.map((u, k) => (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img key={k} src={u} alt="" className="h-auto max-w-full self-start rounded-[0.5em]" />
                        ))}
                      </div>
                    ) : null;
                  }
        ...
        {belowFields.map((f) => {
          ...
          return hasContent(v) ? (
            <div key={String(f.key)} className="relative mt-[0.8em]">
              ...
              <p
                className="whitespace-pre-line py-[1.1em] pl-[1.2em] pr-[1.2em] text-[0.875em] leading-[1.8] text-[#6A7282]"
                style={textStyle}
...
function Box({
  ...
  return (
    <div className={full ? "col-span-2" : ""}>
      <div
        className={`flex items-center gap-[0.5em] ${BLOCK_STYLE.valueBox} ${bold ? "font-bold" : ""}`}
...
function AirLegCard({ label, airline, place, date, time, desc }: { label: string; airline?: string; place?: string; date?: string; time?: string; desc?: string }) {
  ...
  return (
    <div className="rounded-[10px] border border-[#2563EB1a] bg-[#2563EB08] px-[1.1em] py-[0.9em]">
      <p className="flex items-center gap-[0.5em] text-[min(1.178vw,22.6195px)] max-[991px]:text-[min(3.0595vw,18.357px)] max-[501px]:text-[2.9708vw] font-bold text-[#2563EB]">
        {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      {place && <p className="mt-[0.4em] text-[min(1.2196vw,23.4178px)] max-[991px]:text-[min(3.1673vw,19.0038px)] max-[501px]:text-[3.0758vw] font-semibold text-[#1E3A5F]"><Html v={place} /></p>}
      {when && <p className="mt-[0.15em] text-[min(1.2473vw,23.9501px)] max-[991px]:text-[min(3.2393vw,19.4358px)] max-[501px]:text-[3.1455vw] font-medium text-[#6A7282]">{when}</p>}
      {hasContent(desc) && <p className="mt-[0.35em] whitespace-pre-line text-[min(1.178vw,22.6195px)] max-[991px]:text-[min(3.0595vw,18.357px)] max-[501px]:text-[2.9708vw] leading-[1.6] text-[#6A7282]"><Html v={desc} /></p>}
    </div>
  );
}
  ...
  return (
    <div className="flex gap-[0.8em]">
      ...
      <div className="min-w-0 flex-1 pt-[0.45em] pb-[0.7em]">
        ...
        {hasContent(b.blockDesc) && <p className="mt-[0.5em] whitespace-pre-line text-[min(1.178vw,22.6195px)] max-[991px]:text-[min(3.0595vw,18.357px)] max-[501px]:text-[2.9708vw] leading-[1.6] text-[#6A7282]"><Html v={b.blockDesc} /></p>}
      </div>
    </div>
  );
}
    ...
    <div className="flex gap-[0.8em]">
      ...
      <div className="min-w-0 flex-1 pt-[0.45em] pb-[0.7em]">
        <div className="rounded-[10px] border border-[#2563EB1a] bg-[#2563EB08] px-[1.1em] py-[0.9em]">
          ...
          {place && <p className="mt-[0.4em] text-[min(1.2196vw,23.4178px)] max-[991px]:text-[min(3.1673vw,19.0038px)] max-[501px]:text-[3.0758vw] font-semibold text-[#1E3A5F]"><Html v={place} /></p>}
          {when && <p className="mt-[0.15em] text-[min(1.2473vw,23.9501px)] max-[991px]:text-[min(3.2393vw,19.4358px)] max-[501px]:text-[3.1455vw] font-medium text-[#6A7282]">{when}</p>}
          {hasContent(b.blockDesc) && <p className="mt-[0.35em] whitespace-pre-line text-[min(1.178vw,22.6195px)] max-[991px]:text-[min(3.0595vw,18.357px)] max-[501px]:text-[2.9708vw] leading-[1.6] text-[#6A7282]"><Html v={b.blockDesc} /></p>}
        </div>
      </div>
    </div>
  );
...
function MealBlockView({ block: b }: { block: ItineraryBlock }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-[1.1em] gap-y-[0.4em] py-[0.4em] text-[min(0.93vw,17.856px)] max-[991px]:text-[min(2.343vw,14.058px)] max-[501px]:text-[2.786vw]">
      <span className="font-bold text-navy">식사</span>
      {items.map(([label, val]) => (
        <span key={label} className="flex items-center gap-[0.4em]">
          <span className="rounded bg-navy/5 px-[0.6em] py-[0.2em] text-[0.85em] font-bold text-navy">{label}</span>
  ...
  if (kind === "관광지") {
    return (
      <div className="py-[0.2em]">
        <p className="text-[min(1.0129vw,19.4475px)] max-[991px]:text-[min(3.0235vw,18.141px)] max-[501px]:text-[2.9358vw] font-bold text-navy">
          <Html v={name} />
          {country && <Html v={country} className="ml-[0.5em] text-[0.85em] font-normal text-slate-400" />}
        </p>
        {hasContent(desc) && (
          <p className="mt-[0.2em] whitespace-pre-line text-[min(1.0534vw,20.2245px)] max-[991px]:text-[min(2.7354vw,16.4124px)] max-[501px]:text-[2.6562vw] leading-[1.7] text-slate-500"><Html v={desc} /></p>
        )}
      </div>
    );
  }
    ...
    <div className="rounded-[0.5em] border border-slate-200 bg-white p-[1em]">
      <div className="flex gap-[1em] max-[501px]:flex-col">
        ...
        <div className="min-w-0 flex-1">
          <p className="flex flex-wrap items-center gap-[0.5em] text-[min(1.1709vw,22.481px)] max-[991px]:text-[min(3.0408vw,18.2448px)] max-[501px]:text-[2.9528vw] font-bold text-navy">
            <Html v={name} />
            <span className="rounded bg-[#1E4D8B] px-[0.5em] py-[0.15em] text-[0.65em] font-semibold text-white">기항지 안내</span>
          </p>
          <p className="mt-[0.4em] text-[min(0.9757vw,18.7334px)] max-[991px]:text-[min(2.5339vw,15.2034px)] max-[501px]:text-[2.4605vw] font-bold text-[#1E4D8B]"><Html v={country} /></p>
          <p className="mt-[0.3em] whitespace-pre-line text-[min(0.927vw,17.7984px)] max-[991px]:text-[min(2.4072vw,14.4432px)] max-[501px]:text-[2.3375vw] leading-[1.6] text-slate-500"><Html v={desc} /></p>
        </div>
      </div>
    </div>
  );
